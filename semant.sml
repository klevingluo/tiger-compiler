...
fun transExp(exp : A.exp, env) =
  let fun findTy(sym, pos, env) = E.lookupTy(sym, env, pos)
      ...
      and writeDecs(dec::restdecs, env) =
          ...
          let fun addFunDecs({name, params, result, body, pos}::fundecs, env) =
                  let val resultTy =
                          case result
                           of SOME((sym, pos)) => findTy(sym, pos, env)
                            | _ => T.UNIT
                      val paramTys = map(fn ({name, escape, typ, pos}) => findTy(typ, pos, env))(params)
                  in (E.setVar(name, E.FunEntry{formals= paramTys, result= resultTy}, env);
                      addFunDecs(fundecs, env))
                  end
                | addFunDecs([], _) = ()
              and checkFunDecs({name, params, result, body, pos}::fundecs, env) =
                  let val resultTy =
                          case result
                           of SOME((sym, pos)) => findTy(sym, pos, env)
                            | _ => T.UNIT
